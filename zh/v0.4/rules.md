...
详细用法参见 [配置参考](./configuration#配置验证)。
...
| `webhook_msg_format` | string | — | 通过 Webhook 发送时覆盖 `msg_format`，支持相同模板变量。 |
| `bot_msg_format` | string | — | 通过 Bot 发送时覆盖 `msg_format`，支持相同模板变量。 |
...
| `{msg}` | 消息文本内容 |
| `{time}` | 消息时间戳（ISO 格式） |
| `{username}` | 平台原生用户名（如适用会带 `@` 前缀） |
| `{nickname}` | 发送者的显示名称（`{user}` 的别名） |
| `{source_mentioned_self}` | 源消息是否提及了机器人自身 |
