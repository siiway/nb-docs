...
2. 复制机器人 Token。
3. 将机器人的 Webhook 地址设置为 `http://<你的服务器><webhook_path>`（例如 `http://1.2.3.4/yunhu-webhook`）。
4. 将机器人添加到你的群组。
...
| `webhook_secret` | 否 | — | 追加到 Webhook URL 的额外路径段，用于安全验证 |
...
| `max_file_size` | 否 | `10485760`（10 MB） | 发送附件时单个文件的最大字节数 |
