...
3. Set the bot's webhook URL to `http://<your-server>:<port><webhook_path>` (e.g. `http://1.2.3.4:8000/yunhu-webhook`). The port should match your shared HTTP server port.
...
| `webhook_secret` | No | — | Secret string appended to the webhook URL as an extra path segment for security |
| `max_file_size` | No | `10485760` (10 MB) | Maximum bytes per attachment when sending |
