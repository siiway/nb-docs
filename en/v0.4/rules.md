...
Use the `validate` command to check syntax correctness of rules:

```sh
# Validate rules at default path
uv run main.py validate

# Validate specific rules file
uv run main.py validate -r path/to/rules.yaml

# Validate rules with custom data directory
uv run main.py validate -d custom-data/
```

See [Configuration Reference](./configuration#validating-configuration) for full usage details.
...
| `webhook_msg_format` | string | — | Overrides `msg_format` when the message is sent via webhook. Supports the same template variables. |
| `bot_msg_format` | string | — | Overrides `msg_format` when the message is sent via bot. Supports the same template variables. |
...
| `{time}` | Message timestamp (ISO format) |
| `{username}` | Platform-native username (with `@` prefix if applicable) |
| `{nickname}` | Sender's display name (alias for `{user}`) |
| `{source_mentioned_self}` | Whether the source message mentioned the bot itself |
